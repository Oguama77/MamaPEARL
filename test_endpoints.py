...
class EndpointTester:
    ...
    def test_endpoint(self, method: str, endpoint: str, data=None, files=None, description: str = ""):
        ...
        try:
            ...
            if result["success"]:
                self.log(f"✅ SUCCESS - {endpoint} ({result['status_code']}) - {result['response_time']:.2f}s")
            else:
                self.log(f"❌ FAILED - {endpoint} ({result['status_code']}) - {result['response_time']:.2f}s", "ERROR")
    ...
    def test_chat_endpoints(self):
        ...
        test_cases = [
            ...
            {
                "data": {"message": "Tell me about preeclampsia risk"},
                "description": "Preeclampsia-related question"
            },
            {
                "data": {"message": "What are the symptoms of high blood pressure?"},
        ...
        for test_case in test_cases:
            result = self.test_endpoint("POST", "/chat", data=test_case["data"], 
    ...
    def test_prediction_endpoints(self):
        ...
        sample_variables = [
            75.0, 165.0, 27.5, 1.0, 0.0, 120.0, 80.0, 98.0, 36.8, 72.0,
            14.2, 4.5, 142.0, 4.2, 98.0, 24.0, 1.2, 45.0, 3.5, 1.8,
            150.0, 8.5, 35.0, 0.9, 7.4, 2.1, 0.8, 12.0, 180.0, 95.0
        ]
        ...
        test_cases = [
            ...
            {
                "data": {"variables": sample_variables[:10]},  # Too few variables
        ...
        results = []
        for test_case in test_cases:
            result = self.test_endpoint("POST", "/predict", data=test_case["data"],
                                      description=test_case["description"])
            results.append(result)
    ...
    def test_variable_endpoints(self):
        ...
        test_cases = [
            {
                "data": "75, 165, 27.5, 1, 0, 120, 80, 98, 36.8, 72, 14.2, 4.5, 142, 4.2, 98, 24, 1.2, 45, 3.5, 1.8, 150, 8.5, 35, 0.9, 7.4, 2.1, 0.8, 12, 180, 95",
        ...
        for test_case in test_cases:
            ...
            result = self.test_endpoint("POST", "/normalize_variables", data=payload,
                                      description=test_case["description"])
            results.append(result)
    ...
    def test_image_endpoints(self):
        ...
        test_cases = [
            {
                "files": {"files": ("test_image.png", dummy_image_content, "image/png")},
                "description": "Upload dummy image file"
            },
            {
                "files": {"files": ("test_document.txt", b"not an image", "text/plain")},
        ...
        results = []
        for test_case in test_cases:
            result = self.test_endpoint("POST", "/extract", files=test_case["files"],
                                      description=test_case["description"])
            results.append(result)
            
        # Test with no file
        result = self.test_endpoint("POST", "/extract", description="No file uploaded")
        results.append(result)
    ...
    def generate_report(self):
        ...
        for category, tests in self.results.items():
            ...
            for i, test in enumerate(tests, 1):
                ...
                response_time = f" ({test['response_time']:.2f}s)" if test.get("response_time") else ""
                ...
                if test.get("success", False):
                    ...
                    elif "response" in test and isinstance(test["response"], dict) and "error" in test["response"]:
                        self.log(f"     API Error: {test['response']['error']}")
                        
                total_tests += 1
                
            self.log(f"  Category Summary: {category_passed}/{category_total} passed")
